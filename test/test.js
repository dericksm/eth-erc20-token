...
contract('dBank', ([deployer, user]) => {
  ...
  beforeEach(async () => {
    ...
    await token.passOwnership(dbank.address, {from: deployer})
  })

  describe('testing token contract...', () => {
    describe('success', () => {
      it('checking token name', async () => {
        expect(await token.name()).to.be.eq("Decentralized Derick's Currency")
      })

      it('checking token symbol', async () => {
        expect(await token.symbol()).to.be.eq('DDC')
      })
      ...
      it('dBank should have Token minter role', async () => {
        expect(await token.owner()).to.eq(dbank.address)
      })
    })

    describe('failure', () => {
      it('passing minter role should be rejected', async () => {
        await token.passOwnership(user, {from: deployer}).should.be.rejectedWith(EVM_REVERT)
      })

      it('tokens minting should be rejected', async () => {
        await token.approveUse(user, '1', {from: deployer}).should.be.rejectedWith(EVM_REVERT)
      })
    })
  })
